...
stf_file_path = './LCSTS/train.json_sft.pkl'
...
def train(n_epochs, batch_size):
    ...
    model = GPT(CFG)
    model = ddp_helper.process_model(model, 'gpt.pth')

    train_data_loader = ddp_helper.create_dataloader(
        dataset=LCSTSDataset(stf_file_path, CFG['ctx_len']),
        ...
        for batch, (inputs, labels) in enumerate(train_data_loader):
            ...
            try:
                ...
                if ddp_helper.is_main_process():
                    ...
                    if batch % 1000 == 0:
                        gen1 = generate(model,'###总结以下内容###输入:一辆小轿车，一名女司机，竟造成9死24伤。日前，深圳市交警局对事故进行通报：从目前证据看，事故系司机超速行驶且操作不当导致。目前24名伤员已有6名治愈出院，其余正接受治疗，预计事故赔偿费或超一千万元。', 100)
                        gen1 = gen1.split('###输出:')[-1]
...
if __name__ == '__main__':
    train(50, 5)
    # train_on_novel(100, 1)
    # test_dataloader()
    # test_novel_data_loader()
    # test_data_loader()
    # test_warmup()
